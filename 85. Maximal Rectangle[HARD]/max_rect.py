...
# For each row build a 'histogram' of the number of 1s above
# and including each column.  Then proceed as for problem 084
        ...
        for row in range(rows):
            ...
            for col in range(1, len(heights)):
                while stack and heights[col] < heights[stack[-1]]:
                    ...
                    ryt_exc_bound = col
                    if stack: left_exc_bound = stack[-1]
                    else: left_exc_bound = -1
                    width = ryt_exc_bound - left_exc_bound - 1
