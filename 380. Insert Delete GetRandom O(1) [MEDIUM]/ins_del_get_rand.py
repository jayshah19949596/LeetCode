...
# Maintain a list of the vals as they are added and
# a mapping from each val to its index in the list.
# Upon removal replace with last val in list (self if removing
# last val), update mapping for replacement val then
# delete mapping for removed val.
# Note that using a set and no list is also O(1) but pop()
# does not produce a random val and random.sample is slow.
# Time - O(1)
# Space - O(n)
